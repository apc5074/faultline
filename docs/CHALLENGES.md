...
**Slug:** `url-shortener`  
**Version:** 3 (bump and republish when simulator semantics change)
...
Challenge-owned fractions (must sum to 1). Phase 3 activates these via `deriveRegionalWorkload`; nearest-healthy routing applies when Services have regional deployments and a Global Router sits on the request path. Geography is absorb-then-route: CDN evaluates redirect hits at edge ingress first, and only post-CDN misses plus writes continue into regional Services. Geo attribution never sends the full 124,000 RPS through the origin path when the CDN absorbs traffic.
...
| `headroom` | Capacity headroom | headroom | 0.10 (10%) | ≥ |
...
## Path-aware Level 1 scoring

The URL Shortener declares a redirect workload completion contract in its Level
Profile. The contract is deliberately topology-neutral: CDN edge hits are a
valid terminal, while origin branches require a Service and a durable data
path. Component traffic alone is not completion evidence.

For example, two Services behind a Load Balancer may both receive traffic. If
only one has a valid Postgres or Redis-origin path, only that branch can
complete redirects. The other Service still contributes configured cost and
may show local load, but its requests become explicit incomplete traffic and
cannot improve throughput, latency, headroom, or hot-key results.

Challenges score simulator outcomes rather than prescribed technologies. A
direct Service → Postgres design, a CDN-backed design, and a read-aside Redis
design may all be competitive when their completed workload, SLOs, and budget
pass. Disconnected extras remain legal so players can explore tradeoffs, but
they provide no workload benefit and still cost money.
