...
export function ExperimentResultPanel({
  ...
  const failedServices = result.events
    ...
    .map((event) => event.componentId!);
  const reroutedConnections = result.events
  ...
  return <section className="experiment-result-panel" aria-label="Simulated experiment result">
    ...
    {coldCaches.map((componentId) => (
      <p key={componentId} className="experiment-result-panel__cold-cache">
        simulated cold cache · {componentId} · this experiment only
      </p>
    ))}
    {failedServices.map((componentId) => (
      <p key={`failed-${componentId}`} className="experiment-result-panel__failure">
        simulated service failed · {componentId} · no automatic repair
      </p>
    ))}
    {reroutedConnections.length > 0 ? (
      <p>authoritative surviving routes: {reroutedConnections.join(", ")}</p>
    ) : null}
    {unmetRps > 0 ? (
    ...
    {cacheMetrics.map((event) => (
      <p key={`cache-${event.componentId}`}>
        {event.componentId}: {metric(event.data.hitRps as number)} hit RPS · {metric(event.data.missRps as number)} miss RPS · {metric((event.data.downstreamAvoidedRps as number | undefined) ?? 0)} downstream RPS avoided
      </p>
    ))}
