...
export async function registerAgentWebMcpSurface(
  ...
  if (signal.aborted) {
    return { registeredToolNames: [], readToolNames: [], visualToolNames: [] };
  }

  const [read, visual] = await Promise.all([
  ...
  return {
    registeredToolNames: [...read.registeredToolNames, ...visual.registeredToolNames],
