...
export type WebMcpRegistrationGroup = "all" | "stable-review" | "stable-visual" | "specialists" | "experiments" | "stable-interview";
...
function abortedResult(group: WebMcpRegistrationGroup): RegisterAgentWebMcpSurfaceResult {
  return { ...(group !== "all" ? { group } : {}), resolvedToolNames: [], registeredToolNames: [], failedToolNames: [], readToolNames: [], visualToolNames: [], experimentToolNames: [], sessionToolNames: [] };
}
...
export async function registerAgentWebMcpSurface(options: RegisterAgentWebMcpSurfaceOptions): Promise<RegisterAgentWebMcpSurfaceResult> {
  const { modelContext, registry, getContext, getCurrentEvidenceRevision, signal, development = false, onVisualIntent, onExperimentResult, onPresentationCue, interviewService, timing, trace, traceGeneration, group = "all" } = options;
  ...
  const [read, visual, experiment, interview] = await Promise.all([
    includeRead
  ...
  const tools = [...readTools, ...visual.tools, ...experiment.tools, ...interviewTools];
  const namesByMode = { read: readTools.map(({ name }) => name), visual: visual.tools.map(({ name }) => name), experiment: experiment.tools.map(({ name }) => name), session: interviewTools.map(({ name }) => name) } as const;
  const manifest: WebMcpRegistrationManifest = {
    ...
    tools,
    namesByMode,
    skipped: [...read.skipped.map(({ name }) => name), ...visual.skipped.map(({ name }) => name), ...experiment.skipped.map(({ name }) => name), ...interview.skipped],
  ...
  return {
    ...(group !== "all" ? { group } : {}),
    resolvedToolNames: [...readTools.map(({ name }) => name), ...visual.tools.map(({ name }) => name), ...experiment.tools.map(({ name }) => name), ...interviewTools.map(({ name }) => name)],
    registeredToolNames,
    failedToolNames,
