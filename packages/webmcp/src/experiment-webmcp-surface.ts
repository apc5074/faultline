...
export interface BuildExperimentWebMcpSurfaceOptions { readonly registry: AgentCapabilityRegistry; readonly getContext: WebMcpContextFactory; readonly development?: boolean; readonly onExperimentResult?: (result: ExperimentResult) => void; }
...
export async function buildExperimentWebMcpSurface(options: BuildExperimentWebMcpSurfaceOptions): Promise<ExperimentWebMcpSurface> {
  const context = resolveLiveAgentSnapshot(await options.getContext()).context;
