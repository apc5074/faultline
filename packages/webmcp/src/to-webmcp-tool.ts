...
function webMcpDescription(capability: RegisteredCapability): string {
  const metadata: Record<string, string> = {
    ...
    get_coaching_policy: "Call once per coaching session on the first turn. Retain policyText, policyDigest, and turnProtocol in host system context; do not call again unless the challenge changes. Pair with get_session_focus in parallel on the first turn only.",
