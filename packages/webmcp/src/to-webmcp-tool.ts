...
function webMcpDescription(capability: RegisteredCapability): string {
  const metadata: Record<string, string> = {
    review_current_design: "Use for overview, diagnosis, focus, revision delta, ambiguity, or errors. Lead with present evidence; mechanism categories are OK, but do not recommend specific catalog components to add. For errors use requirement_failure; production zooms like inspect_component.",
    ...
    get_coaching_policy: "Call once per coaching session on the first turn. Retain policyText, policyDigest, turnProtocol, and prohibitedActions in host system context; do not call again unless the challenge changes. Pair with get_session_focus in parallel on the first turn only.",
    get_session_focus: "Read human canvas focus and pending help. Call when focus or pending help may have changed.",
