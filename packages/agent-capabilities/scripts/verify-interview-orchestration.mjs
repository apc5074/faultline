...
for (const tool of ["start_design_interview", "submit_interview_answer", "follow_up_design_interview", "advance_design_interview", "prepare_interview_simulation_review", "submit_interview_simulation_critique"]) {
...
assert.match(prompt, /canvas redesign/i);
assert.match(prompt, /exact.*reviewDigest/i);
assert.match(prompt, /Chat prose alone can never complete/i);
