...
import { createDefaultCapabilityRegistry } from "../dist/index.js";
...
const result = await registry.invoke("inspect_bottlenecks", context, {});
assert.equal(result.ok, true);
assert.deepEqual(result.data.risks.map((risk) => risk.kind), ["saturation", "headroom", "latency", "unmet_demand", "hot_key", "budget"]);
const unavailable = await registry.invoke("inspect_bottlenecks", { ...context, simulation: { available: false, validationErrors: ["invalid"] } }, {});
assert.deepEqual(unavailable, { ok: false, code: "SIMULATION_UNAVAILABLE", message: "invalid" });
console.log("inspect_bottlenecks capability verification passed");
