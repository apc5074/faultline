...
function dynamicCapabilities(context: AgentContext): Set<string> {
  const types = new Set(context.architecture.components.map((component) => component.type));
  return new Set([...(types.has("redis") || types.has("cache") ? ["inspect_cache"] : []), ...(context.architecture.components.some((component) => component.type === "postgres" && Number(component.config.readReplicaCount ?? 0) > 0) ? ["inspect_replication"] : []), ...(context.architecture.components.some((component) => new Set(component.deployments.map((deployment) => deployment.regionId)).size > 1) ? ["inspect_regional_traffic"] : [])]);
}
...
export function buildReviewRevisionDelta(previous: AgentContext, current: AgentContext): ReviewRevisionDelta {
  ...
  const changedComponentIds = [...currentComponents].filter((id) => previousComponents.has(id) && JSON.stringify(componentById(previous, id)) !== JSON.stringify(componentById(current, id))).sort();
...
export function buildReviewCurrentDesignOutput(context: AgentContext, input: ReviewCurrentDesignInput, session: AgentSessionState): CapabilityResult<ReviewCurrentDesignOutput> {
  ...
  if (intent === "cost_review") {
    if (context.reviewPackets) return capabilityOk({ ...common, cost: { monthlyTotal: context.reviewPackets.cost.monthlyTotal ?? 0, budget: context.reviewPackets.cost.budget, overBudget: (context.reviewPackets.cost.monthlyTotal ?? 0) > context.reviewPackets.cost.budget, topContributors: context.reviewPackets.cost.topContributors }, suggestedNextTools: [suggested("inspect_component", "Connect the largest deterministic contributor to system evidence."), suggested("get_metrics", "Check whether cost pressure aligns with observed behavior.")] });
    const cost = getCostBreakdown(context);
